...
export function SubmitPage({ orders, selected, mode, date, onDateChange, onBack, onFilter, onToggle, onToggleAll, onDetail, onPrepare }: { orders: WorkOrder[]; selected: string[]; mode: "manual" | "historical"; date: string; onDateChange: (value: string) => void; onBack: () => void; onFilter: () => void; onToggle: (id: string) => void; onToggleAll: () => void; onDetail: (order: WorkOrder) => void; onPrepare: () => void }) {
  ...
  return <><header className="topbar"><div><p className="eyebrow">步骤 2 · {mode === "historical" ? "历史到访不遇" : "手动图片提交"}</p><h1>选择提交工单</h1></div><div className="topbar-actions"><button className="text-button" onClick={onBack}>上一步</button><FilterButton onClick={onFilter} /><DatePicker value={date} onChange={onDateChange} /></div></header><div className="stepper workflow-steps"><span>1</span><i /><span className="current">2</span><i /><span>3</span><i /><span>4</span><i /><span>5</span><em>选择</em></div><section className="summary-strip submit-summary"><span><b>{selected.length}</b> 单已选</span><span className="summary-dot" /><span>可继续勾选</span><button className="select-all" onClick={onToggleAll}>{orders.length > 0 && selectedInView === orders.length ? "取消全选" : "一键全选"}</button></section><WorkOrderList orders={orders} selected={selected} onToggle={onToggle} onDetail={onDetail} />{selected.length > 0 && <button className="submit-fab" onClick={onPrepare} aria-label={`准备并提交 ${selected.length} 个工单`}><Icon name="send" size={23} /><span>{selected.length}</span></button>}</>;
}
