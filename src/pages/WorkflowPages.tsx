...
function UploadFilePicker({
  ...
  return <div className="workflow-file-picker"><label className="file-picker"><span>{file?.name || placeholder}</span><input key={inputKey} type="file" accept="image/*" onChange={(event) => { const nextFile = event.target.files?.[0] ?? null; event.currentTarget.value = ""; if (nextFile) setPendingFile(nextFile); }} /></label>{url ? <button type="button" className="upload-photo-preview" onClick={() => setPreviewOpen(true)} aria-label={`放大预览${label}`}><img src={url} alt={label} /><span><b>{label}</b><small>点击放大预览</small></span><Icon name="search" size={18} /></button> : null}{previewOpen && url ? <ImagePreviewDialog fileName={file?.name || label} label={label} url={url} onClose={() => setPreviewOpen(false)} /> : null}{pendingFile && pendingUrl ? <ImagePreviewDialog fileName={pendingFile.name} label={`${label}候选图片`} url={pendingUrl} onClose={() => setPendingFile(null)} onConfirm={() => { onPick(pendingFile); setPendingFile(null); }} /> : null}</div>;
}
...
export function PreparePage({ orders, activeOrder, submitMode, historyFile, selectedHistoryPhotoId, historyMode, historyPhotos, historyPhotoError, historyPhotoLoading, historyPhotoStatusByOrder, libraryFile, detailCloseupFile, allManualReadyByOrder, date, onDateChange, onBack, onSelectOrder, onPickHistoryFile, onHistoryModeChange, onFindHistoricalPhotos, onSelectHistoricalPhoto, onPickLibraryFile, onPickDetailCloseupFile, onNext }: { orders: WorkOrder[]; activeOrder: WorkOrder; submitMode: BatchSubmitMode; historyFile: File | null; selectedHistoryPhotoId: string; historyMode: "manual" | "auto"; historyPhotos: HistoryPhotoCandidate[]; historyPhotoError: string | null; historyPhotoLoading: boolean; historyPhotoStatusByOrder: Record<string, HistoryPhotoStatus>; libraryFile: File | null; detailCloseupFile: File | null; allManualReadyByOrder: Record<string, boolean>; onBack: () => void; onSelectOrder: (id: string) => void; onPickHistoryFile: (file: File | null) => void; onHistoryModeChange: (mode: "manual" | "auto") => void; onFindHistoricalPhotos: () => void; onSelectHistoricalPhoto: (photo: HistoryPhotoCandidate) => void; onPickLibraryFile: (file: File | null) => void; onPickDetailCloseupFile: (file: File | null) => void; onNext: () => void } & DateProps) {
  ...
  return <><SubHeader title="选择提交图片" date={date} onDateChange={onDateChange} onBack={onBack} /><WorkflowSteps current={2} /><section className="prepare-tabs">{orders.map((order, index) => {
    ...
    const statusText = allManual ? (allManualReadyByOrder[order.id] ? "两张已选" : "待选两张") : statusLabels[status];
  ...
  })}</section><section className="prep-order"><p className="eyebrow">{activeOrder.id}</p><h2>{activeOrder.address}</h2><span>{activeOrder.resident} · {activeOrder.unit} · {activeOrder.time}</span></section><section className="section-block"><div className="section-heading"><div><h3>{allManual ? "门牌照片" : "历史到访照片"}</h3><p>{allManual ? "当前工单单独选择，不会沿用上一单" : "选择一张已核验的“到访不遇”图片"}</p></div><b>必选</b></div>{!allManual && <div className="photo-source-tabs"><button type="button" className={historyMode === "manual" ? "active" : ""} onClick={() => onHistoryModeChange("manual")}>手动上传</button><button type="button" className={historyMode === "auto" ? "active" : ""} onClick={() => onHistoryModeChange("auto")}>自动拉取</button></div>}{allManual || historyMode === "manual" ? <UploadFilePicker file={historyFile} inputKey={`${activeOrder.id}-primary`} label={allManual ? "门牌照片" : "历史到访照片"} placeholder={allManual ? "选择当前工单门牌照片" : "选择历史照片"} onPick={onPickHistoryFile} /> : <div className="auto-history-source"><p>{batchLoadingCount > 0 ? `正在批量拉取所选工单，剩余 ${batchLoadingCount} 单处理中…` : historyPhotoLoading ? "正在自动核验历史工单与到访不遇日志…" : "所选工单已批量拉取，可直接切换卡片选择照片。"}</p>{historyPhotoError ? <><p className="history-photo-error">{historyPhotoError}</p><button type="button" className="auto-history-search" onClick={onFindHistoricalPhotos}>重新查询</button></> : null}{historyPhotos.length > 0 && <div className="history-photo-grid">{historyPhotos.map((photo) => <button type="button" key={photo.id} className={selectedHistoryPhotoId === photo.id ? "selected" : ""} onClick={() => onSelectHistoricalPhoto(photo)}><img src={photo.previewUrl} alt={photo.originalName} /><span>{photo.originalName}</span><small>历史单 {photo.historyLabel}</small></button>)}</div>}</div>}</section><section className="section-block"><div className="section-heading"><div><h3>{allManual ? "到访不遇详细单近景照片" : "本机图库补图"}</h3><p>{allManual ? "当前工单单独选择，切换工单后必须重新上传" : "选择一张现场环境图片"}</p></div><b>必选</b></div><UploadFilePicker file={allManual ? detailCloseupFile : libraryFile} inputKey={`${activeOrder.id}-secondary`} label={allManual ? "到访不遇详细单近景照片" : "本机图库补图"} placeholder={allManual ? "选择当前工单详细单近景照片" : "选择本机补图"} onPick={allManual ? onPickDetailCloseupFile : onPickLibraryFile} /></section><div className="page-action"><button type="button" className="primary-button" onClick={onNext}>下一步：确认提交 <Icon name="chevron" size={17} /></button></div></>;
}

function FilePreview({ file, label }: { file: File | null | undefined; label: string }) {
  const url = useFilePreviewUrl(file);
  const [previewOpen, setPreviewOpen] = useState(false);
  return <>{<button type="button" className="file-preview" disabled={!url} onClick={() => setPreviewOpen(true)} aria-label={url ? `放大预览${label}` : `${label}未选择`}>{url ? <img src={url} alt={label} /> : <span>未选择</span>}<small>{label}</small></button>}{previewOpen && url ? <ImagePreviewDialog fileName={file?.name || label} label={label} url={url} onClose={() => setPreviewOpen(false)} /> : null}</>;
}

export function ConfirmPage({ orders, submitMode, historyFiles, libraryFile, detailCloseupFiles, reason, remark, operatorName, operatorLocked, date, onDateChange, onBack, onReasonChange, onRemarkChange, onOperatorNameChange, onStart }: { orders: WorkOrder[]; submitMode: BatchSubmitMode; historyFiles: Record<string, File>; libraryFile: File | null; detailCloseupFiles: Record<string, File>; reason: string; remark: string; operatorName: string; operatorLocked: boolean; onBack: () => void; onReasonChange: (value: string) => void; onRemarkChange: (value: string) => void; onOperatorNameChange: (value: string) => void; onStart: () => void } & DateProps) {
  const allManual = submitMode === "all-manual";
  return <><SubHeader title="确认提交" date={date} onDateChange={onDateChange} onBack={onBack} /><WorkflowSteps current={3} /><section className="confirm-hero"><span>本次将串行处理</span><strong>{orders.length}</strong><b>个到访不遇工单</b><p>每单上传两张照片后关闭工单，再写入流转日志。</p></section><section className="confirm-list">{orders.map((order, index) => <article key={`${order.id}-${index}`}><div><b>{order.resident} · {order.unit}</b><span>{order.id}</span></div><div className="mini-photos"><FilePreview file={historyFiles[order.id]} label={allManual ? "门牌" : "历史"} /><FilePreview file={allManual ? detailCloseupFiles[order.id] : libraryFile} label={allManual ? "详细单近景" : "补图"} /></div></article>)}</section><section className="form-card"><label>流转人<input value={operatorName} readOnly={operatorLocked} placeholder="请输入流转人" onChange={(event) => onOperatorNameChange(event.target.value)} /></label><label>流转原因<input value={reason} onChange={(event) => onReasonChange(event.target.value)} /></label><label>备注<textarea value={remark} onChange={(event) => onRemarkChange(event.target.value)} placeholder="可选，默认不填写" /></label></section><p className="safety-note">提交后不可撤销。关闭成功但日志失败时，系统只会补发日志，不会重复关闭。</p><div className="page-action"><button type="button" className="primary-button" onClick={onStart}>开始串行提交 <Icon name="play" size={16} /></button></div></>;
}
