...
export function ModePage({ mode, intervalMinSeconds, intervalMaxSeconds, onModeChange, onIntervalMinChange, onIntervalMaxChange, onBack, onNext }: { mode: BatchSubmitMode; intervalMinSeconds: number; intervalMaxSeconds: number; onModeChange: (value: BatchSubmitMode) => void; onIntervalMinChange: (value: number) => void; onIntervalMaxChange: (value: number) => void; onBack: () => void; onNext: () => void }) {
  ...
  return <><header className="subheader"><button className="back-button" onClick={onBack} aria-label="返回"><Icon name="chevron" /></button><h1>批量提交设置</h1><div /></header><WorkflowSteps current={1} /><section className="workflow-intro"><p className="eyebrow">步骤 1</p><h2>手动上传与提交间隔</h2><span>当前仅开放手动上传模式；工单将按顺序执行，间隔只在相邻两单之间生效。</span></section><section className="mode-options"><button type="button" disabled><b>手动图片提交</b><span>已停用：不再共用批次补图</span><small>已停用</small></button><button type="button" disabled><b>历史到访不遇</b><span>已停用：不再自动拉取历史照片</span><small>已停用</small></button><button type="button" className={mode === "all-manual" ? "active" : ""} onClick={() => onModeChange("all-manual")}><b>手动上传模式</b><span>自动勾选并带入已预填门头照的工单，其余照片按工单逐张选择并校验重复</span></button></section><section className="interval-setting"><div><b>工单提交间隔</b><span>避免连续请求过快，下一单会随机等待</span></div><div className="interval-slider" aria-label="工单提交间隔"><output aria-live="polite">{intervalSummary}</output><div className="interval-dual-range" style={intervalRangeStyle}><input type="range" min="1" max="20" value={intervalMinSeconds} onChange={(event) => onIntervalMinChange(Number(event.target.value))} aria-label="最短提交间隔" /><input type="range" min="1" max="20" value={intervalMaxSeconds} onChange={(event) => onIntervalMaxChange(Number(event.target.value))} aria-label="最长提交间隔" /><span className="interval-range-thumb is-min" aria-hidden="true" /><span className="interval-range-thumb is-max" aria-hidden="true" /></div><div className="interval-scale" aria-hidden="true"><span>1 秒</span><span>20 秒</span></div></div></section><div className="page-action"><button type="button" className="primary-button" onClick={onNext}>下一步：选择工单 <Icon name="chevron" size={17} /></button></div></>;
}
